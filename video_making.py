...
def create_tts_files(client, txt_n, input_voice, temp_page):
    current_dir = os.path.dirname(os.path.abspath(__file__))
    audio_dir = os.path.join(current_dir, "audio_files")    
    # audio_files 폴더가 없으면 생성
    if not os.path.exists(audio_dir):
        os.makedirs(audio_dir)
    output_audio_path = os.path.join(audio_dir, f"{temp_page}.wav")
    text_to_speech(client, txt_n, output_audio_path, input_voice)
...
def main(face_path,audio_path):
    global full_frames, mel_chunks, model, detector, predictions, boxes
    args.face = face_path
    process_image(args.face, 'output.png', 400)
    ...
    result_filename = f'results/{audio_filename}.mp4'
    command = 'ffmpeg -y -i {} -i {} -strict -2 -q:v 1 {}'.format(audio_path, 'temp/result.avi', result_filename)
